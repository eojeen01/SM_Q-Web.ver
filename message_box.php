...
<?php
   ...
   for ($i=$start; $i<$start+$scale && $i < $total_record; $i++)
   {
	  ...
	  $result2 = mysqli_query($con, "select id from members where id='$msg_id'");
	  $record = mysqli_fetch_array($result2);
	  $msg_name     = $record["id"];	  
	  $sql2 = "select * from members where id='$msg_id'";
...
?>
				<li>
					...
					<span class="col3"><?=$msg_name?> [lv.<?=$level?>]</span>
   ...
   if ($po=="up")
			{
				...
				$sql = "select * from members where id='$point_id'";
				
				$result = mysqli_query($con, $sql);
				$row = mysqli_fetch_array($result);
				$new_point = $row["point"] + $point_up;
	
				$sql = "update members set point=$new_point where id='$point_id'";
